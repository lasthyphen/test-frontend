...
A delegator stakes DJTX and specifies a validator (the delegatee) to validate on their behalf. The
...
Add a validator to the Primary Network. You must stake DJTX to do this. If the node is sufficiently
...
more DJTX staked and delegated to it than this value.
This value will initially be set to `min(5 * amount staked, 3M DJTX)`.
The total value on a validator is 3 million DJTX.
...
Send DJTX from an address on the Method Chain to an address on the Value Chain or Utility Chain. After issuing this
transaction, you must call the Value Chain's [`avm.import`](value-chain#avmimport) or Utility Chain's
[`djtx.import`](utility-chain#djtximport) with assetID `DJTX` to complete the transfer.
...
Get the amount of nDJTX staked by a set of addresses. The amount returned does not include staking
rewards.
...
Utility Chain’s [`djtx.export`](utility-chain-api#djtxexport) method with assetID `DJTX` to initiate the
transfer.
...
- `to` is the ID of the address the DJTX is imported to. This must be the same as the `to` argument
