...
export const meta = {
  id: 'value-chain-api',
  title: 'Enterprise Consortia Chain API',
  description: 'This guide explains how to make calls to APIs exposed by Dijets nodes.',
}

All Enterprise Consortia Chains (ECCs) support standard Ethereum APIs by default. Dijets Specific APIs in an ECC start with
`djtx` ECCs have the following additional APIs.

## `eth_feeConfig`

An ECC comes with an API request for getting fee config at a specific block. You can use this
...
This API exists on the Utility Chain as well, but in addition to the normal Chain Config returned by the
Utility Chain `eth_getChainConfig` on `subnet-evm` additionally returns the upgrade config, which specifies
