...
Suppose, account Alice wants to send 1.3 DJTX to account Bob, then it has to include all
...
are only including outputs containing `DJTX` assets. These conditions are
...
async function sendBaseTx() {
  ...
  // creating outputs of 0.5 (multisig) and 0.1 DJTX - change output will be added by the function in the last
  let outputConfig = [
    {
      amount: new BN(5e8),
      owners: addresses.x,
...
- Multi-sig output of value 0.5 DJTX with threshold 2 and owners represented by
  `addresses.x`. The owners are basically an array of addresses in Buffer
  representation.
- Single owner output of value 0.1 DJTX.
...
async function exportXP() {
  ...
  // creates mutlti-sig (0.1 DJTX) and single-sig (0.03 DJTX) output for exporting to P Address (0.001 DJTX will be fees)
  let outputConfig = [
    {
      amount: new BN(3e6),
