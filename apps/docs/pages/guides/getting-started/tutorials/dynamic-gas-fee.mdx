...
Each unit of gas is paid for in DJTX at the `gas price` for the transaction. The
...
Consider the following examples (here Gwei or nDJTX equates to one-billionth of 1 DJTX)
...
price that the network supports (as of now, it's 25 nDJTX or Gwei). Although the
...
1. Let's create a sample transaction on Dijets Mainnet, in which we will be
   sending 0.1 DJTX to a receiver using the MetaMask. By clicking **Next** we
...
1. If the network activity is high, you have to edit the priority and max fees
   ...
   average max fee is 31.25 Gwei (nDJTX).
