...
const NavigationMenu = () => {
  ...
  function handleRouteChange(url: string) {
    switch (url) {
      ...
      case url.includes(`/docs/guides/storage`) && url:
        ...
        break
      case url.includes(`/docs/guides/dijetsjs`) && url:
        menuState.setMenuLevelId('dijetsjs')
        break
      case url.includes(`/docs/guides/nodes`) && url:
        menuState.setMenuLevelId('nodes')
        break
      case url.includes(`/docs/guides/developer-toolchains`) && url:
        menuState.setMenuLevelId('developer_tools')
        break
      case url.includes(`/docs/guides/node-apis`) && url:
        menuState.setMenuLevelId('nodeapis')
        break
  ...
  const isNodeApisActive = 'nodeapis' === level
  const isStorageActive = 'storage' === level
  const isDeveloperToolsActive = 'developer_tools' === level
  const isDijetsJsActive = 'dijetsjs' === level
  const isNodesActive = 'nodes' === level
  ...
  return (
    <div className={['flex relative', 'justify-center lg:justify-start'].join(' ')}>
      ...
      <NavigationMenuGuideList id={'nodeapis'} active={isNodeApisActive} />
      <NavigationMenuGuideList id={'realtime'} active={isRealtimeActive} />
      <NavigationMenuGuideList id={'developer_tools'} active={isDeveloperToolsActive} />
      <NavigationMenuGuideList id={'storage'} active={isStorageActive} />
      <NavigationMenuGuideList id={'dijetsjs'} active={isDijetsJsActive} />
      <NavigationMenuGuideList id={'nodes'} active={isNodesActive} />
